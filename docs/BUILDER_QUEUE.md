...
**Last updated:** 2026-09-10 by Builder — **item 29 is COMPLETE and v0.1.14 is RELEASED
...
⚠⚠ **ISSUE #31 IS NOT FIXED, AND MUST NOT BE CLOSED.** The field is now *sent* and
*stored* — that is all that is established. **Whether the player draws a browse list from
it is still unproven**, no Yoto document says it does, and Yoto's own description of the
field says *"used in the app"*, not the player. **The hardware test has not happened.**
A card that plays in the phone app is not evidence. `HOW-TO-TEST-THE-CARDS.md` now carries
a **Part 2b** asking the knob question directly on `1WCvI` (5 chapters) and telling the
tester that `gzP2B` cannot answer it (1 chapter — nothing to browse under any hypothesis).
That file stays **untracked** by instruction.
