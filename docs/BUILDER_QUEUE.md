...
**Last updated:** 2026-09-10 by Builder — **item 29 is CLAIMED and in flight on
`feat/overlay-labels`.** Item 28 is on `main` (`1d158e9`), so the dependency is cleared.
Three commit stacks in order **1 → 2 → 3**, shipping as **v0.1.14** (this arc owns the
bump — it corrects ADR §3.6's *"no version bump"*). **The live `--apply` to the three
cards happens inside this cycle**, after merge, under the staged protocol: dry-run all
three, apply `gzP2B` alone, prove rollback end-to-end on it, then `1WCvI`, then `7FcVe`.
Baseline **411 passed**. Item 30 untouched.
